...
config = PeftConfig.from_pretrained("../CultureBank-Mixtral-SFT")
# base_model = AutoModelForCausalLM.from_pretrained("mistralai/Mixtral-8x7B-Instruct-v0.1")
# text_model = PeftModel.from_pretrained(base_model, "/../../../mnt/localssd/CultureBank-Mixtral-DPO")
base_model = AutoModelForCausalLM.from_pretrained("mistralai/Mixtral-8x7B-Instruct-v0.1", device_map="auto")
model = PeftModel.from_pretrained(base_model, "../CultureBank-Mixtral-SFT", device_map="auto")
...
years = [1972, 1977, 1982, 1987, 1991, 1996, 2002, 2006, 2010, 2018]
...
for year in years:
    ...
    for keys in tqdm(quest_dict.keys()):
        ...
        try:
            ...
            if(len(ref)>=50 or len(ques.split(' '))<=5):
                continue
        
            # prompt = 'Question: Please tell us whether you are satisfied or dissatisfied with your social life.\nA. Very satisfied\nB. Somewhat satisfied\nC. Somewhat dissatisfied\nD. Very dissatisfied\nAnswer:'
        
            prompt = f'Answer the following question as a person living in the year {year}.\nQuestion: {ques}'
            counter = ord('A') 
            l = []
            for val in ref:
            ...
            survey_data[year].append({'question':ques,'prompt':prompt, 'options':ref,'log_prob':log_probs})
        
            import json
        
            with open(f'results-steer/GSS_{model_name.split("/")[-1]}.json','w') as f:
